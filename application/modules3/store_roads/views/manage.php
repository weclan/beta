...
<?php
	$create_road_url = base_url()."store_roads/create";
?>
		...
		<div class="m-form m-form--label-align-right m--margin-top-20 m--margin-bottom-30">
			<div class="row align-items-center">
				...
				<div class="col-xl-4 order-1 order-xl-2 m--align-right">
					<a href="<?= $create_road_url ?>" class="btn btn-accent m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
						<span>
							<i class="la la-plus-square"></i>
							<span>
								Tambah Kategori Jalan
							</span>
						</span>
					</a>
		...
		<table class="m-datatable" id="html_table" width="100%">
			...
			<tbody>
				...
				foreach ($query->result() as $row) { 
			  		$edit_road = base_url()."store_roads/create/".$row->id;
			  		$status = $row->status;
				...
				<tr>
					...
					<td data-field="Actions" class="m-datatable__cell">
						<span style="overflow: visible; width: 110px;">						
							...
							<a href="<?= $edit_road ?>" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit details">							
						...
						<div class="modal fade" id="<?= $row->id ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									...
									echo form_open('store_roads/delete/'.$row->id, $attributes);
									?>
